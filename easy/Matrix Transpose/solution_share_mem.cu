...
__global__ void matrix_transpose_kernel(const float* input, float* output, int M, int N) {
  ...
  const int tid = by * TILE + bx;
  
  const int input_block_row_st = by * TILE;
  const int input_block_col_st = bx * TILE;
  ...
  const int output_thread_row = output_block_row_st + ty;
  const int output_thread_col = output_block_col_st + tx;

  __shared__ float S_output[TILE][TILE];

  // 转置读取 global mem to share mem
  if (input_thread_row < M && input_thread_col < N) {
    S_output[tx][ty] = input[(input_thread_row) * N + input_thread_col];
  }
  __syncthreads();

  if (output_thread_row < N && output_thread_col < M) {
    output[(output_thread_row) * M + output_thread_col] = S_output[ty][tx];
  }
}
