...
__global__ void matrix_multiplication_kernel(float* A, float* B, float* C, int M, int K, int N) {
  ...
  sdata_A[0][ldg_A_col * 4 + 1][ldg_A_row] = load_reg_a[1];
  sdata_A[0][ldg_A_col * 4 + 2][ldg_A_row] = load_reg_a[2];
  sdata_A[0][ldg_A_col * 4 + 3][ldg_A_row] = load_reg_a[3];
  FETCH_FLOAT4(sdata_B[0][ldg_B_row][ldg_B_col * 4]) = FETCH_FLOAT4(load_reg_b[0]);
  __syncthreads();
  ...
  do {
    ...
    for (int i = 0; i < BLOCK_K - 1; ++i) {
      FETCH_FLOAT4(reg_a[(i+1)%2][0]) = FETCH_FLOAT4(sdata_A[load_idx][i+1][smem_A_row]);
      FETCH_FLOAT4(reg_a[(i+1)%2][4]) = FETCH_FLOAT4(sdata_A[load_idx][i+1][smem_A_row + 4]);
      FETCH_FLOAT4(reg_b[(i+1)%2][0]) = FETCH_FLOAT4(sdata_B[load_idx][i+1][smem_B_col]);
      FETCH_FLOAT4(reg_b[(i+1)%2][4]) = FETCH_FLOAT4(sdata_B[load_idx][i+1][smem_B_col + 4]);
      #pragma unroll
    ...
    if (smem_tile_idx < K) {
      ...
      sdata_A[write_idx][ldg_A_col * 4 + 1][ldg_A_row] = load_reg_a[1];
      sdata_A[write_idx][ldg_A_col * 4 + 2][ldg_A_row] = load_reg_a[2];
      sdata_A[write_idx][ldg_A_col * 4 + 3][ldg_A_row] = load_reg_a[3];
      FETCH_FLOAT4(sdata_B[write_idx][ldg_B_row][ldg_B_col * 4]) = FETCH_FLOAT4(load_reg_b[0]);
      write_idx ^= 1;
    }
    __syncthreads();
    load_idx ^= 1;
